...
class TestKittiDataset(unittest.TestCase):
    def _generate_kitti_dataset_config(self):
        ...
        pipeline = [
            dict(type='LoadPointsFromFile',
                 coord_type='LIDAR'),
            dict(type='PackMultiTaskInputs', task_handlers={key: dict(type='PackInputs', keys=['points']) for key in ['x','y','z','a','b','c']}, multi_task_fields=['gt_label'])
    ...
    def test_getitem(self):
        DefaultScope.get_instance('test_simple_encoder', scope_name='point_loc')
        np.random.seed(0)
